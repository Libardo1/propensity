...
#' 
#' @export simulation.lee
#' 
#' @examples
#' # Lee used a continuous response of the y propensity score
#' lee <- simulation.lee(n=500, sim="B", cont=TRUE)
#' 
...
#' setoguchi <- simulation.lee(n=2000, sim="B", cont=FALSE)
#' \dontrun{
...
#'    function(scn){
#'      sapply(1:1000, function(ind){simulation.lee(n=n, sim=scn)})
#'    })
#'   
#'   
#'
simulation.lee <- function(n=2000, sim = c("A","B", "C", "D", "E", "F", "G"), cont=TRUE, seed, plot.it=FALSE){
  nvar = 10
